...
for (i in 1:K) {
  ...
  numFolds <- trainControl(method = "cv", number = 3)
   ...
   cpGrid <- expand.grid(.cp = seq(0.01, 0.2, 0.01),.depth =c(0:50))
  
  train_dt <- train(Class~., data = train, method = "rpart", metric="Accuracy",
                    trControl = numFolds, tuneGrid = cpGrid)
  cps[i] <- train_dt$bestTune$cp
  depths[i]<-train_dt$bestTune$depth
  
  tree <- rpart(Class ~ ., train, method="class", cp = cps[i],maxdepth =depths[i])
...
write(c("Best depths ", depths), append = TRUE, file = "DT_result.txt")


accuracies <- rep(0, K)
for(i in 1:K) {
  rf <- rpart(Class~., data = Train_dt, method="class", cp = cps[i],maxdepth =depths[i]) 
  pred_dt <- predict(rf, Test_dt[,-1], type="class")
  cf_dt <- table(Test_dt$Class, pred_dt)
  accuracies[i] <- sum(diag(cf_dt))/sum(cf_dt)
  write(paste("Acc with cp = ", cps[i], ": "," maxdepth = ", depths[i], ": ", sum(diag(cf_dt))/sum(cf_dt)), append = TRUE, file = "DT_result.txt")
